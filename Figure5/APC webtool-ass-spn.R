...
# 获取所有zip文件
dir <- "F:/BaiduSyncdisk/AMRGBD/第一篇/xiaoming/2.APC网页版/Population"
...
df <- df %>% dplyr::filter(!(year %in% c(1990, 1991)))
unique(df$age_name)
# 筛选不同年龄组的数据
...
df <- df %>% mutate(year=case_when(
  year<1997 ~ "1992~1996",
  year<2002 & year>=1997 ~ "1997~2001",
  year<2007 & year>=2002 ~ "2002~2006",
  year<2012 & year>=2007 ~ "2007~2011",
  year<2017 & year>=2012 ~ "2012~2016",
  year<2022 & year>=2017 ~ "2017~2021"))

df2 <- df %>%
  dplyr::filter(pathogen=="Streptococcus pneumoniae" & sex_name== "Both") %>% 
...
## 将数据转换成行为年龄，列为年的数据
apc_case <- df2 %>% dplyr::filter(pathogen=="Streptococcus pneumoniae" & sex_name== "Both")
...
write.csv(result,'apc_web_data_ass_spn.csv')
