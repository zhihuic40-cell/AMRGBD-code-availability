...
df2 <- df %>%
  dplyr::filter(pathogen=="Serratia spp." & sex_name== "Both") %>% 
...
## 将数据转换成行为年龄，列为年的数据
apc_case <- df2 %>% dplyr::filter(pathogen=="Serratia spp." & sex_name== "Both")
...
write.csv(result,'apc_web_data_ass_ser.csv')
