...
file_path <- here("data", "attnumber19902021.csv")
...
p3 <- ggplot(plot_data, aes(x = location_id, y = deathcounts_mean, fill = pathogen)) +
  ...
  scale_y_continuous(
    ...
    breaks = seq(0, 250, by = 50)  # 增加了纵坐标刻度，每2.5K显示一个刻度
  ) +
  labs(
    x = "Location",
  ...
  theme(
    ...
    panel.grid.major.y = element_line(color = "grey90", linewidth = 0.3),  # 添加水平网格线
...
# Display the plot
print(p3)
ggsave("Figure1C.pdf", p3, width = 10.68, height = 7.1, units = "in",
       device = cairo_pdf)