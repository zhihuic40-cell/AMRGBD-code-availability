DRAW_TIMEOUT_SECONDS <- suppressWarnings(as.numeric(Sys.getenv("FIGURE6_DRAW_TIMEOUT_SECONDS", "180")))
if (!is.finite(DRAW_TIMEOUT_SECONDS) || DRAW_TIMEOUT_SECONDS <= 0) {
...
if (!exists("DIAGNOSTIC_LOG_FILE", inherits = FALSE)) {
...
resolve_fixed_param_file <- function(pathogen_name, counterfactual_type) {
  output_base <- paste0(gsub(" ", "_", pathogen_name), "_", counterfactual_type)
  filename <- paste0(output_base, "_optimal_parameters.csv")

  rel_dirs <- c(
    file.path("results", "Figure6"),
    file.path("..", "results", "Figure6"),
    file.path("..", "..", "results", "Figure6")
  )

  candidates <- file.path(rel_dirs, filename)
...
run_single_draw_subprocess <- function(draw_matrix,
                                       population_extended_final,
                                       optimal_config,
                                       wstand,
                                       expected_years,
                                       prediction_years,
                                       timeout_sec) {
  callr::r(
    func = function(draw_matrix,
                    population_extended_final,
                    optimal_config,
                    wstand,
                    expected_years,
                    prediction_years) {
      normalize_prediction_years_inner <- function(raw_years, expected_years) {
        raw_years <- as.numeric(raw_years)
        expected_years <- as.numeric(expected_years)
        if (length(raw_years) != length(expected_years)) {
          stop("Raw prediction years and expected years must have the same length.")
      ...
      extract_prediction_means_inner <- function(bapc_result, expected_years, target_years) {
        if ("ASR" %in% names(bapc_result)) {
          predicted_asr <- as.data.frame(bapc_result$ASR) * 10^5
        } else {
          predicted_asr <- as.data.frame(BAPC::agestd.rate(bapc_result)) * 10^5
        ...
        predicted_asr$year <- normalize_prediction_years_inner(
          raw_years = rownames(predicted_asr),
          expected_years = expected_years
        )
        predicted_asr <- predicted_asr[predicted_asr$year %in% target_years, c("year", "mean"), drop = FALSE]
      ...
      draw_years <- rownames(draw_matrix)
      ...
      extended_death_matrix[draw_years, ] <- as.matrix(draw_matrix)
      ...
      extract_prediction_means_inner(
    ...
    args = list(
      ...
      population_extended_final = population_extended_final,
...
run_input_uncertainty_analysis <- function() {
  ...
  diag_log("DRAW_TIMEOUT_SECONDS=", DRAW_TIMEOUT_SECONDS)
  ...
  diag_log("Section markers found: section4=", section4_idx, ", section5=", section5_idx, ", section7=", section7_idx)
  ...
  diag_log("Loaded fixed parameters from ", fixed_selection$param_file, "; gf=", fixed_selection$optimal_params$gf, "; secondDiff=", fixed_selection$optimal_params$secondDiff)
  ...
  cat("gf:", fixed_selection$optimal_params$gf, "secondDiff:", fixed_selection$optimal_params$secondDiff, "\n\n")
  ...
  results_dir_figure6 <- get("results_dir", envir = analysis_env)

  if (dir.exists("results")) {
    results_dir <- file.path("results", "Sensitivity", "TableS9")
  } else if (dir.exists(file.path("..", "results"))) {
    results_dir <- file.path("..", "results", "Sensitivity", "TableS9")
  } else if (dir.exists(file.path("..", "..", "results"))) {
    results_dir <- file.path("..", "..", "results", "Sensitivity", "TableS9")
  } else {
    results_dir <- file.path(dirname(dirname(results_dir_figure6)), "Sensitivity", "TableS9")
  }
  ...
  diag_log("Prepared model objects; expected_years=", paste(expected_years, collapse = ","), "; history years=", paste(range(full_years), collapse = "-"))
  ...
  diag_log("Prepared input data; history_input rows=", nrow(history_input), "; actual_asr rows=", nrow(actual_asr))
  ...
  for (draw_idx in seq_len(INPUT_DRAW_COUNT)) {
    ...
    draw_result <- tryCatch({
      ...
      draw_stage <- "subprocess_bapc"
      diag_log("DRAW ", draw_idx, " BAPC START timeout=", DRAW_TIMEOUT_SECONDS)
      draw_values <- run_single_draw_subprocess(
        draw_matrix = draw_matrix,
        population_extended_final = population_extended_final,
        optimal_config = optimal_config,
        wstand = wstand,
        expected_years = expected_years,
        prediction_years = prediction_years,
        timeout_sec = DRAW_TIMEOUT_SECONDS
      )
      diag_log("DRAW ", draw_idx, " BAPC DONE values=", paste(sprintf("%.4f", draw_values), collapse = ","))
      draw_values
    }, error = function(e) {
      diag_log("DRAW ", draw_idx, " ERROR stage=", draw_stage, " message=", conditionMessage(e))
      cat("Skipping input draw", draw_idx, "due to error:", conditionMessage(e), "\n")
      rep(NA_real_, length(prediction_years))
    })

    predicted_draw_matrix[draw_idx, ] <- draw_result
    diag_log("DRAW ", draw_idx, " END stored=", paste(ifelse(is.na(draw_result), "NA", sprintf("%.4f", draw_result)), collapse = ","))
  }
  ...
  asr_with_period <- asr_with_period[, c("RMSE", "MAE", "Periods", "Observed", "Predicted", "Absolute difference (95%CI)", "Relative change (95%CI)")]
