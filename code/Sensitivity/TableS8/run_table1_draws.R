...
for (type in types) {
  for (p in PATHOGENS) {
    draw_file <- file.path(paths$draw_data_dir, "table1", sprintf("table1_%s_%s_draws.rds", type, p$short))
    ...
    if (length(target_results) > 0) {
      ...
      target_csv <- file.path(paths$draw_results_dir, sprintf("table1_%s_%s_netdrift_draws.csv", type, p$short))
    ...
    if (length(target_failures) > 0) {
      ...
      fail_csv <- file.path(paths$draw_results_dir, sprintf("table1_%s_%s_failures.csv", type, p$short))
...
if (length(all_results) > 0) {
  ...
  raw_csv <- file.path(paths$draw_results_dir, "table1_netdrift_draws_all.csv")
  write.csv(raw_draws, raw_csv, row.names = FALSE)

  summary_df <- summarize_net_drift_draws(
    draw_results = raw_draws,
    group_cols = c("type", "pathogen", "location", "cause", "measure", "sex", "rei")
  )
  summary_csv <- file.path(paths$results_dir, "table1_draw500_netdrift_summary.csv")
  write.csv(summary_df, summary_csv, row.names = FALSE)

  cat(sprintf("\n✓ Table1 draw-level summary saved: %s (%d rows)\n", summary_csv, nrow(summary_df)))
} else {
  cat("\n[WARNING] No Table1 draw-level results generated.\n")
}

if (length(all_failures) > 0) {
  failure_df <- bind_rows(all_failures)
  failure_csv <- file.path(paths$results_dir, "table1_draw500_failures.csv")
  write.csv(failure_df, failure_csv, row.names = FALSE)
  cat(sprintf("✓ Table1 draw-level failures saved: %s (%d rows)\n", failure_csv, nrow(failure_df)))
}
