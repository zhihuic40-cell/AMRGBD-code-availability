...
if (is_code_ocean) {
  ...
  if (dir.exists("data/Figure4")) {
    # Running from root directory
    data_dir <- "data/Figure4"
    results_dir <- "results/Figure4"
  } else if (dir.exists("../../data/Figure4")) {
    # Running from code/Figure4/
    data_dir <- file.path("..", "..", "data", "Figure4")
    results_dir <- file.path("..", "..", "results", "Figure4")
  } else {
    # Fallback: use here package
    data_dir <- here("data", "Figure4")
    results_dir <- here("results", "Figure4")
  }
}
if (!dir.exists(results_dir)) dir.create(results_dir, recursive = TRUE)

# Load RDS files
apc_web_data_ass_global_output <- readRDS(file.path(data_dir, "apc_web_data_ass_global_output.rds"))
apc_web_data_ass_CEEECA_output <- readRDS(file.path(data_dir, "apc_web_data_ass_CEEECA_output.rds"))
apc_web_data_ass_HI_output <- readRDS(file.path(data_dir, "apc_web_data_ass_HI_output.rds"))
apc_web_data_ass_LAC_output <- readRDS(file.path(data_dir, "apc_web_data_ass_LAC_output.rds"))
apc_web_data_ass_NAME_output <- readRDS(file.path(data_dir, "apc_web_data_ass_NAME_output.rds"))
apc_web_data_ass_SA_output <- readRDS(file.path(data_dir, "apc_web_data_ass_SA_output.rds"))
apc_web_data_ass_SAEAO_output <- readRDS(file.path(data_dir, "apc_web_data_ass_SAEAO_output.rds"))
apc_web_data_ass_SSA_output <- readRDS(file.path(data_dir, "apc_web_data_ass_SSA_output.rds"))

apc_web_data_att_global_output <- readRDS(file.path(data_dir, "apc_web_data_att_global_output.rds"))
apc_web_data_att_CEEECA_output <- readRDS(file.path(data_dir, "apc_web_data_att_CEEECA_output.rds"))
apc_web_data_att_HI_output <- readRDS(file.path(data_dir, "apc_web_data_att_HI_output.rds"))
apc_web_data_att_LAC_output <- readRDS(file.path(data_dir, "apc_web_data_att_LAC_output.rds"))
apc_web_data_att_NAME_output <- readRDS(file.path(data_dir, "apc_web_data_att_NAME_output.rds"))
apc_web_data_att_SA_output <- readRDS(file.path(data_dir, "apc_web_data_att_SA_output.rds"))
apc_web_data_att_SAEAO_output <- readRDS(file.path(data_dir, "apc_web_data_att_SAEAO_output.rds"))
apc_web_data_att_SSA_output <- readRDS(file.path(data_dir, "apc_web_data_att_SSA_output.rds"))
...
y_max <- ceiling(max(sapply(ranges, function(x) x[2]), na.rm = TRUE) / 500) * 500
...
# Save figure
output_file <- file.path(results_dir, "Figure4_age_effect.pdf")
ggsave(output_file, final_plot, width = 25, height = 11)
