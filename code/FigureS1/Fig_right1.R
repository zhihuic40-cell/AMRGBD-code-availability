...
all_data_ranges <- list(
  get_data_range(cohort_rr_aba_att), get_data_range(cohort_rr_cit_att),
  get_data_range(cohort_rr_ent_att), get_data_range(cohort_rr_eco_att),
  get_data_range(cohort_rr_kpn_att), get_data_range(cohort_rr_pae_att),
...
for (i in 1:length(pathogens)) {
  ...
  filename <- file.path(results_dir, paste0("FigureS1_PMFOUR", i, clean_name, ".pdf"))
