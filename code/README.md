...
│
├── Figure5/          # Pathogen-specific APC patterns (Associated deaths)
│   ├── Fig_left.R        # Age distribution by pathogen
...
│   └── data/             # Pathogen-specific APC outputs (ASS)
│
...
│
├── FigureS1/         # Pathogen-specific APC patterns (Attributable deaths)
│   ├── Fig_left.R        # Age distribution by pathogen
│   ├── Fig_middle1.R     # Local drift by pathogen
│   ├── Fig_middle2.R     # Age effects by pathogen
│   ├── Fig_middle3.R     # Period effects by pathogen
│   ├── Fig_right1.R      # Cohort effects by pathogen
│   └── data/             # Pathogen-specific APC outputs (ATT)
│
└── Sensitivity/      # Monte Carlo sensitivity analyses (Tables S8–S9)
    ├── README.md     # Detailed documentation
    ├── TableS8/      # APC net drift with input data uncertainty (500 draws)
    └── TableS9/      # BAPC counterfactual with input data uncertainty (500 draws)
```
...
1. Open `AMRGBD.Rproj` in RStudio — this automatically sets the working directory to the repository root.
...
```r
# From the repository root (where AMRGBD.Rproj lives):
source("code/Figure1/Figure1A.R")
...
- All scripts auto-detect the working directory and locate data files accordingly
- Data files are stored in `data/` with subdirectories mirroring the `code/` structure
...
- **Monte Carlo sensitivity analysis**: To propagate GRAM input data uncertainty into APC and BAPC models (500 draw-level reruns; see `Sensitivity/`)
...
- **Figure 5**: Pathogen-specific APC decomposition results for 7 bacterial species (associated deaths)
- **Figure S1**: Pathogen-specific APC decomposition results for 7 bacterial species (attributable deaths)
