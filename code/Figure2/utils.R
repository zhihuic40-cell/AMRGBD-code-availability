...
filter_death_rate_data <- function(data, year = 2021) {
  ...
  if ("Measure" %in% names(data)) {
    result <- result %>% dplyr::filter(Measure == "Deaths")
  }
  
  if ("Metric" %in% names(data)) {
    result <- result %>% dplyr::filter(Metric == "Rate (per 100k)")
  }
  
  if ("Age" %in% names(data)) {
    result <- result %>% dplyr::filter(Age == "Age-standardized")
  }
  ...
  # Return result (even if empty, but not NULL)
  if (nrow(result) == 0) {
