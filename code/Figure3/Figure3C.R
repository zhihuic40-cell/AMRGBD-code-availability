...
# Read Excel file
file_path <- file.path(data_dir, "ATTproportion.xlsx")
...
# Save plot as PDF and PPT format
# Save as PDF (works on all platforms)
output_pdf <- file.path(results_dir, "Figure3C.pdf")
ggsave(output_pdf, final_plot_with_x, width = 18, height = 10, units = "cm", dpi = 300, bg = "white")
cat(sprintf("✓ PDF saved to: %s\n", output_pdf))

# Save as PPT (optional, requires export package)
tryCatch({
  output_ppt <- file.path(results_dir, "Figure3C.pptx")
  graph2ppt(x = final_plot_with_x, file = output_ppt, width = 18/2.54, height = 10/2.54)
  cat(sprintf("✓ PPT saved to: %s\n", output_ppt))
}, error = function(e) {
  cat("Note: PPT export skipped (export package may not be available)\n")
})
