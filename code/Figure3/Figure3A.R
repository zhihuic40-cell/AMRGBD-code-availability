...
if (is_code_ocean) {
  ...
  if (dir.exists("data/Figure3")) {
    # Running from root directory
    data_dir <- "data/Figure3"
    results_dir <- "results/Figure3"
  } else if (dir.exists("../../data/Figure3")) {
    # Running from code/Figure3/
    data_dir <- file.path("..", "..", "data", "Figure3")
    results_dir <- file.path("..", "..", "results", "Figure3")
  } else {
    # Fallback: use here package
    data_dir <- here("data", "Figure3")
    results_dir <- here("results", "Figure3")
  }
}
...
# Save figure
output_file <- file.path(results_dir, "Figure3A_mortality_trends.pdf")
ggsave(output_file, final_plot, width = 22, height = 10, dpi = 300, bg = "white")
