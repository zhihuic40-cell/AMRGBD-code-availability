...
if (is_code_ocean) {
  ...
  if (dir.exists("data/Figure6")) {
    ...
    data_dir <- "data/Figure6"
    results_dir <- "results/Figure6"
  } else if (dir.exists("../data/Figure6")) {
    # Running from code/ directory
    data_dir <- file.path("..", "data", "Figure6")
    results_dir <- file.path("..", "results", "Figure6")
  } else {
    # Running from code/Figure6/ or using here()
    data_dir <- file.path("..", "..", "data", "Figure6")
    results_dir <- file.path("..", "..", "results", "Figure6")
  }
}
...
# Set pathogen and counterfactual for analysis
PATHOGEN_NAME <- 'Enterobacter spp.'
COUNTERFACTUAL_TYPE <- 'Attributable'
...
EC <- read.csv(file.path(data_dir, "data.csv"))
EC2 <- EC

# Modify data filtering logic based on data split strategy
EC_full <- EC %>% dplyr::filter(counterfactual == COUNTERFACTUAL_TYPE & measure == "Deaths" & pathogen == PATHOGEN_NAME)
...
age_stand <- read.csv(file.path(data_dir, 'GBD2021 world population age standard.csv'))[-c(1:3),]
...
gbd_pop_dir <- file.path(data_dir, "GBD_population")
dirname <- dir(gbd_pop_dir)
file <- paste0(gbd_pop_dir, "/", dirname)
...
GBD_population_prediction <- fread(file.path(data_dir, 'IHME_POP_2017_2100_POP_REFERENCE_Y2020M05D01.CSV')) %>%
...
optimize_bapc_parameters <- function(max_combinations = 30) {
  cat("Starting BAPC parameter optimization for", PATHOGEN_NAME, "-", COUNTERFACTUAL_TYPE, "...\n")
...
EC_actual <- EC2 %>% 
  dplyr::filter(counterfactual == COUNTERFACTUAL_TYPE & 
               measure == "Deaths" & 
             pathogen == PATHOGEN_NAME &
...
# Create figure with optimized sizing and margins
p <- ggplot(ep_data, aes(x = year)) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = 14, face = "bold", margin = margin(t = 15, b = 5)),
    axis.title.y = element_text(size = 14, face = "bold", margin = margin(r = 15, l = 5)),
  ...
  geom_line(aes(y = actual_rate), color = "black", linewidth = 1.2) +
  geom_line(aes(y = predicted_rate), color = "red", linewidth = 1.2, linetype = "solid") +
  geom_vline(xintercept = 2020, linetype = "dashed", linewidth = 0.8, color = "gray40") +
  geom_vline(xintercept = 2021, linetype = "dashed", linewidth = 0.8, color = "gray40") +
  labs(
    title = paste(PATHOGEN_NAME, "-", COUNTERFACTUAL_TYPE),
    x = "Year",
    y = "Age-standardized mortality rate (per 100,000)"
  ) +
  scale_x_continuous(breaks = seq(2010, 2021, 2), limits = c(2010, 2021.5)) +
  scale_y_continuous(limits = c(NA, upper_limit), expand = expansion(mult = c(0.05, 0.1)))

print(p)

# Save plot to results directory with larger dimensions
plot_filename <- paste0(gsub(" ", "_", PATHOGEN_NAME), "_", COUNTERFACTUAL_TYPE, "_prediction_plot.pdf")
plot_file <- file.path(results_dir, plot_filename)
ggsave(plot_file, p, width = 12, height = 8, units = "in", device = cairo_pdf, dpi = 300)
...
# Create unique output filenames including counterfactual
